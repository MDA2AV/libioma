...
st, hd, body = read_response(s)
s.close()
results.append(check("PUT /users/:id -> 405 with both methods of that path, in registration order",
                     st == 405 and hd.get("allow") == "GET, POST"))
...
results.append(check("a 5 KB reply header -> the reply cannot be built; the connection closes",
                     rs == [] and raw_exchange.closed))
