...
goes with a JSON writer that writes in place, a static directory kept in memory per worker,
response compression as middleware (brotli, gzip), raw pipes for other protocols, TLS 1.3 from a
certificate store, and slices for everything the request carries - compared, searched (SIMD),
cut.
